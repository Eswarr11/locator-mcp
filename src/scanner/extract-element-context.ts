...
export async function extractAllElementContexts(
  ...
  return page.evaluate(
    ({ selector, maxDepth, ignoredTags, testingAttributeNames }) => {
      ...
      const getTestingAttributes = (el: Element): Record<string, string> => {
      ...
      const resolveAriaLabelledBy = (el: Element): string | null => {
      ...
      const getPrecedingLabel = (el: Element): string | null => {
      ...
      const getSiblingIndex = (el: Element): number | null => {
        const parent = el.parentElement;
